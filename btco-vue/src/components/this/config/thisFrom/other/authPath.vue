...
export default {
    methods: {
        save() {
            ...
            if (!this.isDev) {
                ...
                this.$http.post('/config?action=set_admin_path', {
                ...
                }, response => this.$copop.warn('修改入口失败，请检查网络或修复面板', 2000))
    ...
    computed: {
        ...
        isPanelAuthPath: {
            get() { return this.panelAuthPath },
            set(v) { this.$store.commit('thisConfig/updatePanelIs', ['panelAuthPath', v]) }
        }
    }
}
</script>
