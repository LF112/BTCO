...
export default {
    methods: {
        ...
        copyToken() {
            const that = this
            this.$copyText(that.isAPI.token).then(R => {
        ...
        refreshToken() {
            ...
            if (!this.isDev){
                ...
                this.$http.post('/config?action=set_token', { t_type: 1 }, { emulateJSON: true }).then(R => {

                    that.$copop.success('已重置，上一个 TOKEN 已失效', 2000)
                    that.isAPI.token = R.data.msg
                    that.$store.commit('thisConfig/updateIsAPI', that.isAPI)

                }, response => this.$copop.warn('重置失败，请检查网络或修复面板', 2500))
        ...
        saveLimitAddr() {
            ...
            if (!this.isDev){
                ...
                this.$http.post('/config?action=set_token', {
                    t_type: 3,
            ...
            }, response => this.$copop.warn('保存失败，请检查网络或修复面板', 2500))
