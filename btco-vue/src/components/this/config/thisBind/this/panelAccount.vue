...
export default {
    methods: {
        close() {
            ...
            setTimeout(() => this.$refs.panelAccount.style.display = 'none', 500)
        ...
        Save() {
            if (!this.isDev)
                if (!this.change) {
                    ...
                    let checks = ['admin888','123123123','12345678','45678910','87654321','asdfghjkl','password','qwerqwer'],
                    ...
                    for(let i = 0; i < pchecks.length; i++)
                        checks.push(
                            pchecks[i] + pchecks[i] + pchecks[i] + pchecks[i] + 
                            pchecks[i] + pchecks[i] + pchecks[i] + pchecks[i]
                        )
                    ...
                    if (this.passwordInput == '' || this.passwordInput.length  < 8)
                        ...
                        this.$copop.warn('面板密码不能为弱口令 ' + isError, 2000)
                            ...
                            this.$copop.infoUse('现在保存更改密码吗', v => {
                                if (v) {
                                    ...
                                    that.$http.post('/config?action=setPassword', {
